...
def clock_shift(exp_id, ref, node):
    if exp_id == 40:
        shifts_ref1 = {1: 0, 2: 2, 3: 5, 4: 8, 5: 11}
        return shifts_ref1[node] - shifts_ref1[ref]
    elif exp_id == 35:
        shifts_ref5 = {1: 3, 2: 3, 3: 3, 4: 3, 5: 0}
        return shifts_ref5[node] - shifts_ref5[ref]
...
def process_auv_line(counters, line):
    ...
    # Save the time of the recvd pkts
    m = re.match('(?P<time>[\d\.]+)Uwpolling_.*RX_DATA_ID_(?P<pkt_id>\d+)_FROM_NODE_(?P<node_id>\d+)', line)
    if m:
        n_id = int(m.group('node_id'))
        pkt_id = int(m.group('pkt_id'))
        t = float(m.group('time'))

        all_times = counters.setdefault('pkt_rx_times', dict())
        node = all_times.setdefault(n_id, dict())
        if pkt_id in node:
            raise RuntimeError('Duplicated pkt id')
        node[pkt_id] = t
...
def process_node_line(counters, line):
    # Save the times of the pkts generated by the app
    m = re.match('(?P<time>[\d\.]+)Uwpolling_NODE\((?P<node_id>\d+)\)::RECV_FROM_U_LAYERS_ID_(?P<pkt_id>\d+)', line)
    if m:
        n_id = int(m.group('node_id'))
        pkt_id = int(m.group('pkt_id'))
        t = float(m.group('time'))

        all_times = counters.setdefault('pkt_tx_times', dict())
        node = all_times.setdefault(n_id, dict())
        if pkt_id in node:
            raise RuntimeError('Duplicated pkt id')
        node[pkt_id] = t
...
for exp_id in EXP_IDS:
    ...
    global_mean = sum(all_delays) / len(all_delays)
    print('Avg. pkt delay = %f s' % global_mean)
    print('')
