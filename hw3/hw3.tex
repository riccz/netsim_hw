...
\begin{itemize}
  \item since the service time is random the delay that a packet will
    ...
    the packets are stored in a FIFO queue and are used at the end of
    the service time to compute the delay,
  \item the service time is not bounded anymore so a certain value $k$
    that it does not exceed with some probability $\alpha$ is chosen,
    in order to set a transient duration after which the delay and
    queue size are recorded. From
    \begin{align}
...
If this system is simulated for 51 values of $b$ letting it run for $5
\cdot 10^5$ time slots, the average delays suffered by each packet are
the ones shown in Fig.~\ref{plot:queue2_delay} along with their 95\%
confidence intervals.
\begin{figure}[htbp]
  \centering
