...
\item By using the algorithm based on the CDF inversion that
  ...
  \item By generating a string of zeros of length $K_i$, geometrically
    ...
    r.v.s are generated using the CDF inversion method.
\end{enumerate}
\begin{table}[h]
  \centering
...
The execution time of the CDF inversion depends on the value of the
generated sample, which is on average $np$, but it does not vary much
in the tested cases. It is quite high compared to the other two
methods for small values of $n$ and it becomes the best when $n =
1000$ and $p = 0.5$. In the case $n = 1000$, $p=0.9$ the algorithm
does not terminate because the probability $\mathrm{pr}(0) = (1-p)^n$ becomes so
small that Matlab represents it with 0.
\section*{3}
